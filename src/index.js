...
function addTaskToCorrectSection({ target }) {
  if (
    ...
    if (inputTextValue === '') {
      //if input text is not have a value  alert
...
function printData() {
  ...
  for (let i of ul) {
    ...
    if (i.classList.contains('in-progress-tasks')) {
      for (let todo of JSON.parse(localStorage.getItem('tasks'))[
        'in-progress'
      ]) {
        const li = document.createElement('li')
        li.textContent = todo
...
async function save() {
  try {
    ...
    const respone = await fetch(
      'https://json-bins.herokuapp.com/bin/6150ea134e0e2c8361b7ddce',
      {
        headers: {
